...
def mu_theory_flat_lcdm(z, H0, Om, n_grid=6000):
...
def main():
    parser = argparse.ArgumentParser(
        description="Inferencia Pantheon+SH0ES: Flat Lambda CDM, M fijo, H0 libre."
    )
    ...
    parser.add_argument(
        "--M-fixed",
        type=float,
        default=-19.291293562581636,
        help="Valor fijo de M."
    )
    ...
    print("\nModelo: Flat Lambda CDM ")
    print("Caso: M fijo, H0 libre")
    print(f"M fijo = {args.M_fixed}")
    ...
    print("Condición: Omega_Lambda = 1 - Omega_M")
    ...
    def loglike(cube, ndim, nparams):
        ...
        H0, alpha, beta, Om = [cube[i] for i in range(ndim)]
        ...
        mu_obs = mB + alpha * x1 - beta * color - args.M_fixed - biasCor

        # Distancia teórica Flat LCDM
        mu_th = mu_theory_flat_lcdm(
            zHD,
            H0=H0,
            Om=Om
        )
        ...
        # Todos los datos se comparan contra el modelo.
