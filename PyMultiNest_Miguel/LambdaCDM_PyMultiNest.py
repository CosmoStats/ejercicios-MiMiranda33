...
def load_pantheon_covariance(filename):
    ...
    if values.size != n * n:
        raise ValueError(
            f"El archivo de covarianza no tiene el tamaño esperado. "
...
def mu_theory_lcdm_nonflat(z, H0, Om, Ode, n_grid=6000):
    ...
    if abs(Ok) < 1.0e-10:
        ...
        sqrtOk = np.sqrt(Ok)
        DM = (C_LIGHT / H0) / sqrtOk * np.sinh(sqrtOk * Iz)

    else:
        sqrtAbsOk = np.sqrt(abs(Ok))
        DM = (C_LIGHT / H0) / sqrtAbsOk * np.sin(sqrtAbsOk * Iz)
...
def save_corner_plot(samples, labels, basename):
    """
    Guarda un corner plot de las muestras posteriores.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Inferencia Pantheon+SH0ES: Lambda CDM no plano, "
            "M fijo"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Número de live points de MultiNest."
    )
    ...
    print(f"Número total de datos = {n_data}")
    ...
    labels = ["H0", "alpha", "beta", "Omega_M", "Omega_Lambda"]
    ndim = len(labels)
    ...
    bounds = {
        ...
        "beta": (1.0, 5.0),
        "Omega_M": (0.01, 1.20),
        "Omega_Lambda": (0.0, 2.50),
    }

    print("\nParámetros inferidos:", labels)
    print("Condición: Omega_k = 1 - Omega_M - Omega_Lambda")
    ...
    def loglike(cube, ndim, nparams):
        ...
        H0, alpha, beta, Om, Ode = [cube[i] for i in range(ndim)]
        ...
        # Evita regiones de curvatura demasiado extremas
        # que pueden producir problemas numéricos.
        if not (-1.5 < Ok < 1.5):
            return -1.0e100
        ...
        # Distancia teórica Lambda CDM no plano
        mu_th = mu_theory_lcdm_nonflat(
            zHD,
            H0=H0,
            Om=Om,
            Ode=Ode
