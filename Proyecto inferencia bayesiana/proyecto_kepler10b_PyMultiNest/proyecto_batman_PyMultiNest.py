...
def batman_model_phase(x, Tc, rp, a_rs, F0, inc_fixed):
...
def save_fit_plot(x_fit, y_fit, yerr_fit, best_params, basename, inc_fixed):
    Tc, rp, a_rs, F0, sigma = best_params

    x_model = np.linspace(-0.12, 0.12, 1000)
    y_model = batman_model_phase(x_model, Tc, rp, a_rs, F0, inc_fixed)
    ...
    plt.plot(
        ...
        y_model,
        lw=2,
        label=fr"batman con $i={inc_fixed}^\circ$"
    )
    ...
    plt.title("Kepler-10 b: ajuste PyMultiNest con batman")
    plt.xlim(-0.12, 0.12)
...
def main():
    parser = argparse.ArgumentParser(
        description="Inferencia de parametros de transito de Kepler-10 b con PyMultiNest usando batman."
    )
    ...
    print("\nModelo: batman con inclinacion fija")
    print("Objeto: Kepler-10 b")
    print("Archivo de datos:", args.data)
    print("Inclinacion fija:", inc_fixed)
    ...
    def loglike(cube, ndim, nparams):
        Tc, rp, a_rs, F0, sigma = [cube[i] for i in range(ndim)]
    ...
    Tc_best, rp_best, a_rs_best, F0_best, sigma_best = q50

    print("\nParametros centrales:")
    print("Tc =", Tc_best)
    print("Rp/Rs =", rp_best)
    print("a/Rs =", a_rs_best)
    print("F0 =", F0_best)
    print("sigma =", sigma_best)
    print("inc fija =", inc_fixed)

    summary_file = result_basename + "summary.txt"

    with open(summary_file, "w") as f:
        f.write("Modelo batman para Kepler-10 b con PyMultiNest\n")
        f.write("Parametros: Tc Rp_Rs a_Rs F0 sigma inc_fixed logZ logZerr\n")
        f.write(
            f"{Tc_best:.12e} "
            f"{rp_best:.12e} "
            f"{a_rs_best:.12e} "
